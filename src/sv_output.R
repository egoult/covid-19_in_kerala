...
SQ<-read.csv("results/SQ_sensitivity_analysis.csv")
EQ<-read.csv("results/EQ_sensitivity_analysis.csv")
IQ<-read.csv("results/IQ_sensitivity_analysis.csv")
RQ<-read.csv("results/RQ_sensitivity_analysis.csv")

hosp<-SQ[,-c(1:3)]+EQ[,-c(1:3)]+IQ[,-c(1:3)]+RQ[,-c(1:3)]
hosp$Time<-SQ[,"x"]+5

write.csv(hosp, "results/hospitalised_cases_uncertainty.csv")

Deaths<-read.csv("results/Death_sensitivity_analysis.csv")
Deaths<-Deaths[,-c(1:2)]
names(Deaths)[1]<-"Time"

write.csv(Deaths, "results/death_uncertainty.csv")
